...
def classify_component_trend(recent_avg: float, prior_avg: float, strong_threshold: float = 0.25) -> str:
    """
    Generic trend classifier for per-week session frequencies or weekly distance.
    Uses relative change where possible, with a fallback for very small priors.
    """
...
def overall_metrics(df: pd.DataFrame, weekly: pd.DataFrame) -> dict:
    ...
    recent_interval_avg = safe_mean(recent_weeks["interval_sessions"])
    prior_interval_avg = safe_mean(prior_weeks["interval_sessions"])

    recent_long_run_avg = safe_mean(recent_weeks["long_runs"])
    prior_long_run_avg = safe_mean(prior_weeks["long_runs"])

    threshold_trend = classify_component_trend(recent_threshold_avg, prior_threshold_avg, strong_threshold=0.25)
    interval_trend = classify_component_trend(recent_interval_avg, prior_interval_avg, strong_threshold=0.25)
    long_run_trend = classify_component_trend(recent_long_run_avg, prior_long_run_avg, strong_threshold=0.25)

    progression_confidence = "high" if weeks_of_data >= 8 else "medium" if weeks_of_data >= 6 else "low"

    progression_flat_count = sum(
        trend == "flat"
        for trend in [volume_trend, threshold_trend, interval_trend, long_run_trend]
    )

    progression_rising_count = sum(
        trend == "rising"
        for trend in [volume_trend, threshold_trend, interval_trend, long_run_trend]
    )
    ...
    return {
        ...
        "prior_interval_avg": round(prior_interval_avg, 2),
        "recent_long_run_avg": round(recent_long_run_avg, 2),
        "prior_long_run_avg": round(prior_long_run_avg, 2),
