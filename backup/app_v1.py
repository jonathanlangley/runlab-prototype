...
why_bullets = build_why_this_matters(metrics, top_signals, focus)
...
impact_line = "If addressed: this should give the training pattern a clearer platform for improvement."
if target_volume_range:
    impact_line = f"If addressed: move weekly volume toward about {target_volume_range[0]}-{target_volume_range[1]} km while keeping the structure controlled."

st.markdown(
    dedent(
        f"""
        <div class="impact-note">
            <strong>If unchanged:</strong> harder sessions may remain under-supported.<br>
            <strong>If addressed:</strong> {impact_line}
        </div>
        """
    ),
    unsafe_allow_html=True,
...
for step in focus.get("prescription", []):
    st.markdown(f"- {step}")

st.caption(f"Suggested timeframe: {focus.get('timeframe', '2-4 weeks')}")

st.divider()

st.markdown("### AI coaching interpretation")
st.markdown(
    dedent(
        f"""
        <div class="ai-box">
...
st.caption("Generated using the AI explanation layer." if used_ai else "Using fallback explanation because no API key was available.")

st.divider()

st.markdown("### Weekly distance")
fig_weekly = build_weekly_distance_chart(df)
st.pyplot(fig_weekly, use_container_width=True)
...
fig_balance = plot_training_balance_with_counts(balance_df)
st.pyplot(fig_balance, use_container_width=True)

display_df = balance_df.copy()
display_df["Ideal %"] = display_df["Ideal %"].map(lambda x: f"{x:.1f}%")
